...
namespace Homeworlds.Tests.Logic
{
	public class BoardStateActions
	{
		...
		{
			i_Original.RemoveColorFromStar(i_ToEdit, i_ColorToRemove);
			Assert.AreEqual(i_Expected, i_Original);
		}
		...
		private static IEnumerable removeColorFromStarCases()
		{
			...
			yield return new object[4] { state.Clone(), medRed, ePipColor.Blue, state.Clone()};

			List<Ship> shipCopy = ships.ToList();
			shipCopy.RemoveAt(3);
			yield return new object[4] { state.Clone(), medRed, ePipColor.Yellow, new BoardState(shipCopy, stars, eBoardLifecycle.Setup, p1, p2) };

			shipCopy = ships.ToList();
			shipCopy.RemoveAll(s => s.Location.Equals(medRed));
			yield return new object[4] { state.Clone(), medRed, ePipColor.Red, new BoardState(shipCopy, new Star[0], eBoardLifecycle.Setup, p1, p2)};

			Homeworld newP2 = new Homeworld(ePipColor.Blue, ePipSize.Small, ePlayer.Player2, false);
			yield return new object[4] { state.Clone(), p2, ePipColor.Red, new BoardState(ships, stars, eBoardLifecycle.Setup, p1, newP2) };

			Homeworld newP1 = new Homeworld(ePipColor.Green, ePipSize.Medium, ePlayer.Player1, true);
			yield return new object[4] { state.Clone(), p1, ePipColor.Red, new BoardState(ships, stars, eBoardLifecycle.Setup, newP1, p2) };
		}
	}
}
